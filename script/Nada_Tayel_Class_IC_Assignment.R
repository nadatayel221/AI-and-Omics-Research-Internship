...
if (gene_expression >30){print("Gene is highly expressed")}
...
data_10 <-read.csv(file.choose())
data_1_copy <- data_1 
data_10_copy <- data_10
# Identify factor columns for each dataset
factor_cols_1 <- c("gender", "diagnosis", "smoker")  # patient_info.csv
factor_cols_10 <- c("height", "gender")             # metadata.csv
# Convert columns to factors in data_1_copy
for (col in factor_cols_1) {
  data_1_copy[[col]] <- as.factor(data_1_copy[[col]])
}
# Convert columns to factors in data_10_copy
for (col in factor_cols_10) {
  data_10_copy[[col]] <- as.factor(data_10_copy[[col]])
}
# Verify conversions
str(data_1_copy)
str(data_10_copy)
...
# }
# Select binary factor columns (e.g., "smoker" from patient_info data)
binary_cols <- c("smoker")
# Convert factor levels to numeric codes using for loop
for (col in binary_cols) {
  data_1_copy[[col]] <- ifelse(data_1_copy[[col]] == "Yes", 1, 0)
}
# Verify conversion
print(head(data_1_copy))
# ----------------------------------------------------------------------------------------------------------------
#  Verification:
